...
@register_criterion("multi2vec", dataclass=ModelCriterionConfig)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if "target" in sample and "mask_indices" in net_output:
            if sample["target"].shape != net_output["mask_indices"].shape:
                ...
                assert sample["target"].shape == net_output["mask_indices"].shape, ("discrete labels shape and source data shape do not match")

            target_list = sample["target"][net_output["mask_indices"]]
            assert target_list.shape[0] == net_output["x_hubert"].shape[0], ("discrete labels shape and source data shape do not match")
            
            classification = F.cross_entropy(net_output["x_hubert"], target_list.long(), reduction='sum')
            net_output["losses"]["classification"] = classification.sum()
